...
@TeleOp
public class SOLOFieldCentricMecanumTeleOp extends LinearOpMode {
    @Override
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            ...
            if (gamepad1.dpadLeftWasPressed() && !(kick.getPosition()<0.8)){
            ...
            if (gamepad1.dpadRightWasPressed() && !(kick.getPosition()<0.8)){
                currentTick+=slotTicks;
                currentIntTick = (int)Math.round(currentTick);
                spindexer.setTargetPosition(currentIntTick);
                spindexer.setMode(DcMotorEx.RunMode.RUN_TO_POSITION);
            ...
            if (gamepad1.aWasPressed() && (!((spindexer.getCurrentPosition()/slotTicks)%120<110) || !((spindexer.getCurrentPosition()/slotTicks)%120>10))){kick.setPosition(0);}// Shoot
